...
% \subsubsection{Cấu trúc thành phần hệ thống}
% Module vận hành theo kiến trúc truyền nhận dữ liệu liên tục dựa trên mô hình không đồng bộ Publish/Subscribe. Để hiện thực hóa luồng xử lý này, cấu trúc mã nguồn được phân tách rõ ràng thành bốn phân hệ công nghệ, phối hợp chặt chẽ từ tầng thiết bị vật lý lên giao diện ứng dụng:

% \begin{itemize}
%     \item \textbf{Mạch vi điều khiểnYolo:Bit (Xử lý trên Kit phần cứng):} Gồm mạch vi điều khiển Yolo:Bit kết nối trực tiếp với các cảm biến vật lý (DHT22 và quang trở LDR). Tầng này chịu trách nhiệm lấy mẫu, số hóa các giá trị môi trường thực tế, đóng gói dữ liệu thô thành chuỗi ký tự viết tắt theo cấu hình định nghĩa trong tệp \texttt{config.yml} (dòng 75--78) và truyền liên tục ra cổng Serial USB.
    
%     \item \textbf{Dịch vụ Python Gateway (Dịch giao thức):} Là chương trình Python chạy nền đóng vai trò trạm trung chuyển trung gian. Luồng \texttt{run.py} (dòng 244--246) liên tục quét và lắng nghe cổng Serial, chuyển tiếp dữ liệu nhận được cho \texttt{controller.py} (dòng 291--333) bóc tách cú pháp, rồi gọi bộ chuyển đổi \texttt{default\_adapter.py} (dòng 53--73) dịch các mã viết tắt sang định dạng JSON trước khi xuất bản (Publish) lên MQTT Broker Mosquitto.
    
%     \item \textbf{Dịch vụ Backend Node.js (Xử lý trung tâm):} Dịch vụ Node.js sử dụng tệp \texttt{mqttService.js} (dòng 152--199) để đăng ký lắng nghe (Subscribe) các chủ đề cảm biến từ Broker, đưa dữ liệu vào bộ đệm tạm thời. Luồng xử lý sau đó điều phối qua định tuyến \texttt{sensor.js}, tệp điều hướng \texttt{sensorController.js} (dòng 5--15) và dịch vụ \texttt{sensorService.js} (dòng 5--8) để đóng gói snapshot dữ liệu hoàn chỉnh và ghi vào MongoDB.
    
%     \item \textbf{Giao diện người dùng Frontend:} Là trang Dashboard hiển thị của ứng dụng Web SPA trong tệp \texttt{Dashboard.js}. Mã nguồn chịu trách nhiệm khởi tạo trạng thái lưu dữ liệu cảm biến (dòng 6--10), xây dựng hàm gọi API (dòng 37--59), chạy bộ định thời tự động \texttt{setInterval} (dòng 75) để liên tục gọi lên endpoint \texttt{GET /api/sensors/latest} mỗi 5 giây nhằm kết xuất đồ họa và cập nhật các thẻ thông số môi trường theo thời gian thực (dòng 128--149).
% \end{itemize}

% \subsubsection{Nguyên lý vận hành và quản lý trạng thái}
% Module duy trì việc cập nhật trạng thái môi trường thông qua cơ chế lưu trữ snapshot đồng bộ. Để tối ưu hóa băng thông truyền thông mạng và dung lượng lưu trữ cơ sở dữ liệu, Backend áp dụng nguyên lý \textit{Thu thập phân tán - Gom cụm đồng bộ (Distributed Ingestion - Synchronous Buffering)}:
% \begin{itemize}
%     \item \textbf{Cơ chế Gom cụm bộ đệm (Sensor Snapshot Buffer):} Do đặc thù các bản tin cảm biến thô được đẩy lên mạng MQTT một cách bất đồng bộ và độc lập theo từng loại thông số (\texttt{temperature}, \texttt{humidity}, \texttt{light}), Backend thiết lập một cấu trúc mảng tạm thời trong bộ nhớ đệm. 
%     \item \textbf{Quản lý điểm nghẽn và Luồng an toàn:} Hệ thống chỉ thực hiện đóng gói và tạo một tài liệu snapshot môi trường hoàn chỉnh vào cơ sở dữ liệu MongoDB khi bộ đệm thu thập đủ cả ba thông số trong cùng một chu kỳ thời gian. Nhằm khắc phục hạn chế treo cứng luồng (Deadlock) khi có sự cố phần cứng tại một nút cảm biến đơn lẻ, hệ thống tích hợp cơ chế \textit{Định thời gian chờ giải phóng (Timeout-based Eviction)}, tự động nội suy dữ liệu cũ hoặc lưu trữ độc lập nếu bộ đệm không được lấp đầy sau một khoảng thời gian quy định.
% \end{itemize}
...
\begin{itemize}
    \item Hệ thống phần cứng Kit Yolo:Bit đo đạc các giá trị vật lý và định kỳ truyền lên Gateway Python qua kết nối cáp USB Serial dạng chuỗi ký tự viết tắt liên tục, ví dụ: \texttt{!T:27.5\#!H:60\#!Lu:100\#}.
...
% \textbf{Kết quả hiện thực}
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{figures/dashboard-mod1.jpg}
%     \caption{Giao diện hiển thị dữ liệu cảm biến môi trường trên Dashboard}
%     \label{fig:dashboard-mod1}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.85\textwidth]{figures/mod1-kit.jpg}
%     \caption{Kit phần cứng Yolo:Bit với cảm biến đo đạc nhiệt độ, độ ẩm và ánh sáng}
%     \label{fig:mod1-kit}
% \end{figure}
...
% \subsubsection{Thành phần liên quan}
% Module vận hành theo kiến trúc hướng sự kiện (Event-Driven Architecture) kết hợp bộ máy thực thi luật dựa trên quy tắc (Rule-based System). Luồng xử lý phân tách rõ ràng thành hai tầng: tự động hóa cục bộ tại biên không phụ thuộc Internet và quản trị cảnh báo tập trung tại Cloud. Cấu trúc mã nguồn được tổ chức chặt chẽ qua bốn phân hệ:

% \begin{itemize}
%     \item \textbf{Mạch vi điều khiểnYolo:Bit (Xử lý trên Kit phần cứng):} Đảm nhận vai trò thu thập tín hiệu thô từ các cảm biến môi trường để đẩy lên Gateway, đồng thời trực tiếp tiếp nhận khung ký tự điều khiển từ kết nối Serial để đóng ngắt các chân chân vật lý điều khiển thiết bị chấp hành.
%     \item \textbf{Gateway Python (Tự động hóa vật lý):} Chương trình Python đóng vai trò bộ máy tự động hóa cục bộ (Edge Automation). Luồng \texttt{controller.py} (dòng 90--113, 327) tiếp nhận dữ liệu từ cảm biến gửi lên, chuyển tiếp qua dịch vụ \texttt{threshold\_service.py} (dòng 142--192) để so sánh thời gian thực với các quy tắc ngưỡng ngày/đêm định nghĩa sẵn trong tệp \texttt{config.yml} (dòng 49--70) và lập tức xuất lệnh Serial điều khiển phần cứng mà không phụ thuộc vào kết nối Internet.
    
%     \item \textbf{Backend Node.js (Cảnh báo an toàn):} Dịch vụ Node.js chịu trách nhiệm quản lý vòng đời cảnh báo an toàn. Tệp \texttt{mqttService.js} (dòng 185--188) lắng nghe dữ liệu, khi phát hiện bộ đệm snapshot môi trường hoàn chỉnh vi phạm ngưỡng tĩnh định nghĩa tại cấu hình \texttt{alertService.js} (dòng 33--44), luồng sẽ chuyển sang \texttt{alertService.js} (dòng 183--252) để thực thi logic sinh thông báo mới, đồng thời gửi thông báo đẩy đến ứng dụng di động của người dùng.
    
%     \item \textbf{Giao diện người dùng Frontend:} Cấu hình trong tệp \texttt{Dashboard.js} đóng vai trò trực quan hóa trạng thái khẩn cấp. Thành phần giao diện sử dụng các biến trạng thái (dòng 13--18) để lắng nghe tín hiệu từ Backend, thực hiện kết xuất đồ họa khu vực cảnh báo đỏ nổi bật trên màn hình và cung cấp nút bấm tác vụ nhanh (dòng 151--183) giúp người dùng giải tỏa (Resolve) cảnh báo thủ công.
% \end{itemize}

% \subsubsection{Nguyên lý vận hành và quản lý trạng thái}
% Module tự động hóa và cảnh báo vận hành dựa trên hai nguyên lý cốt lõi, đảm bảo tính chính xác từ tầng phần cứng vật lý cho đến tầng quản trị dữ liệu trung tâm:

% \begin{itemize}
%     \item \textbf{Cơ chế thu thập và điều khiển tại biên (Edge Hardware Logic):} Vi điều khiển Yolo:Bit đọc dữ liệu số từ cảm biến DHT22 qua giao thức One-Wire và chuyển đổi tín hiệu tương tự (Analog) từ quang trở sang dải phần trăm ($0-100\%$). Khi nhận lệnh điều khiển thiết bị, do dòng điện các chân GPIO rất yếu, vi điều khiển sẽ kích hoạt qua các mạch đệm công suất trung gian: xuất mức logic cao ($3.3\text{V}$) để đóng mạch kích hoạt quạt/đèn, hoặc băm xung PWM để điều khiển chính xác góc quay động cơ Servo đóng/mở rèm.
%     \item \textbf{Cơ chế chống trùng lặp cảnh báo (Alert De-duplication):} Khi thông số môi trường vượt ngưỡng, Backend sẽ kiểm tra bộ sưu tập \texttt{alerts}. Nếu phát hiện cảnh báo cùng loại vẫn đang mở (\texttt{isResolved: false}), hệ thống sẽ tự động bỏ qua bản tin mới thay vì chèn liên tục vào MongoDB, giúp tối ưu hóa hiệu năng cơ sở dữ liệu và tránh làm tràn ngập các thông báo đỏ trên giao diện người dùng.
% \end{itemize}
...
%     \caption{Giao diện hiển thị cảnh báo ngưỡng trên Dashboard}
%     \label{fig:ui-alert}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=4in]{figures/mod2-fan.jpg}
%     \caption{Hệ thống tự động bật quạt khi nhiệt độ vượt ngưỡng an toàn (trên 30$^{\circ}$C)}
%     \label{fig:mod2-fan}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.85\textwidth]{figures/mod2-led.jpg}
%     \caption{Hệ thống tự động bật đèn LED khi cường độ ánh sáng vượt ngưỡng an toàn (dưới 30 lx)}
%     \label{fig:mod2-led}
% \end{figure}

\subsubsection{Trạng thái cài đặt}
Hoàn thành một phần (Đã hiện thực hóa luồng xử lý tự động hóa cục bộ tại Gateway dựa trên khoảng trễ an toàn, cơ chế đồng bộ và xử lý cảnh báo trùng lặp trên Dashboard; tính năng cấu hình ngưỡng động ngày/đêm và đẩy thông báo đám mây FCM đang nằm trong lộ trình phát triển).

\subsubsection{Hạn chế và hướng phát triển}
Mặc dù hệ thống cơ bản đã vận hành ổn định, Module 2 vẫn tồn tại điểm hạn chế khi tập cấu hình ngưỡng điều kiện hiện tại còn mang tính chất tĩnh. Do được nạp trực tiếp từ tệp tin cấu hình cố định và chưa tích hợp các logic kiểm tra thời gian thực, hệ thống chưa thể tự động chuyển đổi linh hoạt các bộ quy tắc an toàn phù hợp với chu kỳ sinh hoạt ngày và đêm của người dùng. Trong giai đoạn phát triển tiếp theo, hệ thống sẽ được bổ sung giao thức NTP hoặc tích hợp module phần cứng RTC để cập nhật thời gian thực, đồng thời xây dựng bộ lập lịch chạy ngầm (Cron Job) trên Gateway nhằm chủ động tự động tải lại (reload) các bộ cấu hình tương ứng theo các khung giờ định sẵn.

Một hạn chế khác nằm ở việc hệ thống chưa tối ưu hóa cơ chế phản hồi khẩn cấp khi hoàn toàn phụ thuộc vào việc gọi API tuần tự từ giao diện Web để cập nhật trạng thái vi phạm, làm mất đi tính tức thời cần có của một giải pháp an ninh chuyên nghiệp. Để giải quyết bài toán này, kiến trúc phần mềm trong tương lai sẽ mở rộng bằng cách nhúng trực tiếp Firebase Admin SDK (FCM) vào lõi Backend Node.js. Cơ chế này sẽ thiết lập một luồng bắt sự kiện bất đồng bộ, cho phép hệ thống tự động phát hiện số liệu vượt biên nguy hiểm và đẩy lập tức một payload thông báo (Push Notification) xuống thẳng thiết bị di động của người dùng với độ trễ tối thiểu.
...
% \subsubsection{Thành phần liên quan}
% Module vận hành theo kiến trúc truyền nhận dữ liệu liên tục dựa trên mô hình không đồng bộ Publish/Subscribe. Để hiện thực hóa luồng tương tác điều khiển thủ công, cấu trúc mã nguồn được phân rã thành bốn phân hệ công nghệ, phối hợp chặt chẽ từ giao diện Web xuống phần cứng chấp hành:

% \begin{itemize}
%     \item \textbf{Mạch vi điều khiển Yolo:Bit (Xử lý trên Kit phần cứng):} Lắng nghe liên tục các chỉ lệnh rút gọn truyền xuống từ cổng Serial UART. Vi điều khiển thực hiện bóc tách chuỗi ký tự để đóng/ngắt các chân Digital kích mạch đệm công suất bật/tắt quạt và đèn LED, hoặc điều chế độ rộng xung PWM để thay đổi chính xác góc quay của động cơ Servo vật lý gánh nhiệm vụ đóng/mở rèm cửa.
%     \item \textbf{Dịch vụ Python Gateway (Dịch giao thức):} Chương trình Python đóng vai trò tiếp nhận thông tin điều khiển từ mạng. Luồng \texttt{controller.py} (dòng 215--290) thực hiện đăng ký lắng nghe (Subscribe) chủ đề điều khiển từ Broker MQTT, tiếp nhận gói tin chỉ lệnh, giải mã nội dung và trực tiếp xuất bản khung dữ liệu xuống cổng truyền Serial vật lý kết nối với Kit.
%     \item \textbf{Dịch vụ Backend Node.js (Xử lý trung tâm):} Dịch vụ Node.js quản lý luồng nghiệp vụ và phân phối chỉ lệnh. Định tuyến \texttt{device.js} (router POST \texttt{/api/devices/control}) tiếp nhận yêu cầu từ Web Client, chuyển hướng qua tệp điều khiển \texttt{deviceController.js} (dòng 24--118) để cập nhật trạng thái mới vào MongoDB, lưu vết lịch sử tương tác, và gọi dịch vụ \texttt{mqttService.js} (dòng 309--339) để đẩy chỉ lệnh điều khiển dạng bản tin xuống MQTT Broker Mosquitto.
%     \item \textbf{Giao diện người dùng Frontend:} Cấu hình giao diện trang quản lý thiết bị (\texttt{/devices}) trong tệp \texttt{DeviceManagement.js} (dòng 46--77, 143--170). Mã nguồn kết xuất đồ họa lưới các thẻ thiết bị (LED, Quạt, Servo) kết hợp hệ thống công tắc gạt (Toggle Switch) trực quan; khi có tương tác, phân hệ gọi các hàm xử lý trong \texttt{api.js} (dòng 25--39) để phát yêu cầu HTTP REST API dạng POST lên Backend.
% \end{itemize}

% \subsubsection{Nguyên lý vận hành và Quản lý trạng thái}
% Hệ thống quản lý trạng thái của các thiết bị điện trong nhà (đèn, quạt, rèm cửa) dựa trên nguyên lý đồng bộ hai chiều nhằm đảm bảo thông tin hiển thị trên giao diện Web luôn khớp 100\% với trạng thái thực tế của phần cứng vật lý, tránh hiện tượng "trạng thái ảo" khi mất kết nối. Cơ chế này được chia thành hai khái niệm cụ thể:

% \begin{itemize}
%     \item \textbf{Trạng thái mong muốn (Desired State):} Khi người dùng click chuột vào nút bật/tắt thiết bị trên màn hình Web, hệ thống sẽ ghi nhận đây là lệnh yêu cầu thay đổi từ phía con người và gửi lệnh này chạy ngầm xuống tầng dưới. Lúc này, giao diện Web sẽ chưa thay đổi trạng thái ngay lập tức mà nằm ở chế độ chờ.
%     \item \textbf{Trạng thái thực tế (Reported State):} Lệnh sau khi truyền qua Backend và Gateway sẽ đến mạch Yolo:Bit để đóng/ngắt Relay vật lý. Khi phần cứng đã thực thi thành công, mạch Yolo:Bit sẽ gửi ngược một chuỗi ký tự xác nhận (ví dụ: \texttt{!F:1\#}) lên lại Gateway và Backend qua topic \texttt{/state}. 
% \end{itemize}

% Ngay khi Backend nhận được tín hiệu phản hồi thực tế này từ phần cứng, giao diện Web mới chính thức cập nhật trạng thái, đổi màu sắc thẻ thiết bị và hiển thị nhãn "Active". Quy trình này giúp hệ thống hoạt động cực kỳ tin cậy, người dùng sẽ không bao giờ bị đánh lừa là thiết bị đã bật trong khi phần cứng thực tế đang bị kẹt hoặc mất kết nối.
...
%     \caption{Giao diện quản lý và điều khiển thiết bị trên Web}
%     \label{fig:ui-devices}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=4in]{figures/mod3-on-led.jpg}
%     \caption{Hệ thống bật đèn LED khi người dùng kích hoạt thủ công qua Web}
%     \label{fig:mod3-on-led}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=4in]{figures/mod3-on-fan.jpg}
%     \caption{Hệ thống bật quạt khi người dùng kích hoạt thủ công qua Web}
%     \label{fig:mod3-on-fan}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.85\textwidth]{figures/mod3-on-servo.jpg}
%     \caption{Hệ thống đóng rèm khi người dùng kích hoạt thủ công qua Web}
%     \label{fig:mod3-on-servo}
% \end{figure}
...
\subsubsection{Hạn chế và hướng phát triển}
Module 3 hoạt động ổn định và có độ trễ phản hồi phần cứng cực thấp nhờ sự kết hợp giữa giao thức MQTT và truyền tải Serial UART. Không phát hiện khoảng trống thiết kế nghiêm trọng nào ảnh hưởng đến trải nghiệm điều khiển thủ công cơ bản. 

Hướng phát triển tiếp theo trước hết là bổ sung cơ chế xác thực mã hóa hai đầu cho khung truyền Serial nhằm ngăn chặn các nguy cơ tấn công tiêm chỉ lệnh trái phép từ các thiết bị phần cứng giả mạo kết nối vật lý vào bus truyền thông tầm gần. Giải pháp kỹ thuật dự kiến là thiết lập một giao thức đóng khung gói tin chặt chẽ có tích hợp thuật toán băm nội dung để tính toán chuỗi kiểm tra lỗi CRC-8 đính kèm vào cuối mỗi thông điệp. Đồng thời, mã nguồn Python trên Edge Gateway và mã nguồn C++ trên mạch vi điều khiển Yolo:Bit sẽ được tích hợp thêm module mã hóa đối xứng trọng lượng nhẹ (như thuật toán XXTEA hoặc AES-CBC với Pre-shared Key). Mọi gói tin điều khiển trước khi đẩy qua cáp Serial buộc phải đi qua hàm mã hóa và chỉ được vi điều khiển thực thi sau khi đã giải mã, xác thực tính toàn vẹn thành công tại biên phần cứng.

Trọng tâm phát triển kế tiếp là mở rộng hệ thống với tính năng điều khiển thiết bị theo nhóm và quản lý kịch bản sinh hoạt ngữ cảnh động (Group \& Scene Control). Tính năng này cho phép người dùng kích hoạt nhanh một chuỗi phối hợp nhiều thiết bị cùng lúc (ví dụ: ngữ cảnh "Rời khỏi nhà" sẽ đồng thời tắt đèn LED, tắt quạt thông gió và đóng rèm cửa Servo). Lộ trình triển khai sẽ thực hiện mở rộng mô hình dữ liệu MongoDB bằng cách xây dựng một bộ sưu tập mới mang tên \texttt{groups} để lưu trữ mảng định danh thiết bị của từng nhóm. Ở tầng Frontend, giao diện Web sẽ cấu hình các nút bấm tác vụ nhanh; khi tiếp nhận tương tác, Backend Express sẽ truy vấn cấu hình tương ứng trong cơ sở dữ liệu và sử dụng các vòng lặp bất đồng bộ để phát đồng thời các bản tin điều khiển xuống các chủ đề MQTT riêng lẻ, giúp đồng bộ trạng thái toàn bộ phần cứng trong một chu kỳ lệnh duy nhất.

Cuối cùng, hệ thống sẽ phát triển thêm một bộ lập lịch tự động và hẹn giờ định thời (Advanced Scheduler \& Timer) giúp người dùng cài đặt thời gian linh hoạt (ví dụ: hẹn giờ tắt điều hòa vào lúc 5 giờ sáng hoặc bật quạt thông gió trong 30 phút). Để hiện thực hóa, trang quản lý tại Frontend sẽ tích hợp bộ chọn giờ trực quan (Time-Picker) từ thư viện Material-UI để người dùng đặt lịch trình hoặc đếm ngược. Khi nhận yêu cầu, Backend Express sẽ sử dụng thư viện \texttt{node-schedule} phối hợp cùng hàng đợi Redis để quản lý các tác vụ chạy ngầm mà không gây quá tải bộ nhớ. Khi đồng hồ hệ thống chạy đến đúng khung giờ cài đặt, tác vụ ngầm này tự động kích hoạt và phát chỉ lệnh xuống mạng MQTT để điều khiển phần cứng thực thi chính xác mà không cần con người can thiệp thủ công.
...
% \subsubsection{Thành phần liên quan}
% Module vận hành theo kiến trúc hướng dòng dữ liệu (Data-driven Architecture) chạy ngầm dưới dạng các dịch vụ bất đồng bộ nhằm tối ưu hóa hiệu năng, đảm bảo tốc độ ghi nhật ký tần suất cao không gây ảnh hưởng đến thời gian phản hồi của ứng dụng Web Client. Cấu trúc hệ thống được phân rã chặt chẽ dựa trên luồng dữ liệu đi qua bốn phân hệ:

% \begin{itemize}
%     \item \textbf{Mạch vi điều khiển Yolo:Bit (Nguồn phát sự kiện):} Mạch vi điều khiển Yolo:Bit liên tục đo đạc và gửi các gói tin cảm biến thô, đồng thời thực thi các lệnh bật/tắt thiết bị vật lý tại biên. Đây là điểm khởi đầu kích hoạt toàn bộ chuỗi sự kiện ghi vết lịch sử chuỗi thời gian (Time-series data).
%     \item \textbf{Dịch vụ Python Gateway (Chuyển tiếp dữ liệu):} Chương trình Python đóng vai trò bộ chặn sự kiện tầm gần (Event Interceptor). Giao diện Serial và luồng mạng MQTT đảm nhận việc bao bọc các sự kiện phần cứng thành các gói tin tin nhắn có cấu trúc JSON để đẩy đồng bộ lên mạng IP.
%     \item \textbf{Dịch vụ Backend Node.js (Xử lý trung tâm):} Đây là hạt nhân điều phối chính của module, sử dụng các cấu trúc định nghĩa Schema của Mongoose gồm \texttt{Sensor.js}, \texttt{Device.js}, \texttt{ControlTrace.js} và \texttt{ThresholdTrace.js} để chuẩn hóa mô hình dữ liệu MongoDB. Tại đây, dịch vụ chạy ngầm \texttt{mqttService.js} (dòng 138, 178) bắt các sự kiện để lưu trữ dữ liệu môi trường định kỳ. Đối với nhật ký tương tác thủ công của con người, tệp \texttt{deviceController.js} (dòng 93--100) và dịch vụ xử lý thoại \texttt{voiceService.js} (dòng 210, 217) sẽ chặn luồng để gọi hàm chèn vết \texttt{ControlTrace}. Đối với các quyết định tự động hóa khi vi phạm ngưỡng, tệp \texttt{alertService.js} (dòng 239--250) sẽ chịu trách nhiệm gọi lệnh chèn vết hệ thống \texttt{ThresholdTrace}.
%     \item \textbf{Giao diện người dùng Frontend:} Đóng vai trò là bộ máy tiêu thụ và trích xuất dữ liệu (Query Client). Mã nguồn Frontend thiết lập các hàm gọi API nội bộ kết nối tới các endpoint tra cứu lịch sử của Express để lấy dữ liệu nhật ký trích xuất từ database, phục vụ tác vụ kết xuất đồ họa biểu đồ xu hướng biến thiên môi trường hoặc bảng lưới hiển thị danh sách vết vận hành cho người dùng cuối.
% \end{itemize}

% \subsubsection{Nguyên lý vận hành và Quản lý trạng thái}
% Module lưu trữ hoạt động theo nguyên lý thụ động hướng sự kiện (Event-driven). Nghĩa là bản thân module này không chủ động tạo ra dữ liệu, mà chỉ đứng sau lắng nghe các hoạt động của toàn hệ thống để phân loại và cất giữ vào các vùng cơ sở dữ liệu riêng biệt:
% \begin{itemize}
%     \item \textbf{Dữ liệu môi trường định kỳ:} Các số đo nhiệt độ, độ ẩm, ánh sáng được gom lại và ghi vào bộ sưu tập \texttt{sensors} kèm nhãn thời gian để theo dõi xu hướng biến thiên của ngôi nhà.
%     \item \textbf{Nhật ký tương tác của người dùng:} Mọi hành vi chủ động từ phía người dùng như click chuột trên Web hay ra lệnh bằng giọng nói đều được lưu vết vào bộ sưu tập \texttt{control\_traces} kèm cờ phân biệt nguồn gốc (\texttt{frontend} hoặc \texttt{api-client}) để phục vụ công tác gỡ lỗi (Debug).
%     \item \textbf{Nhật ký tự động hóa của hệ thống:} Khi các luật tự động từ cảm biến tự kích hoạt thiết bị phần cứng (ví dụ tự bật quạt khi nóng quá mức), sự kiện này được ghi nhận riêng biệt vào bộ sưu tập \texttt{threshold\_traces} để người dùng biết máy móc đã tự xử lý sự cố lúc nào.
% \end{itemize}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{figures/mongo-device.png}
%     \caption{Giao diện MongoDB Compass hiển thị các tài liệu lưu vết lịch sử tương tác và tự động hóa}
%     \label{fig:mongo-device}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{figures/mongo-sensors.png}
%     \caption{Giao diện MongoDB Compass hiển thị các tài liệu lưu vết lịch sử cảm biến môi trường}
%     \label{fig:mongo-sensors}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{figures/mongo-threshold.png}
%     \caption{Giao diện MongoDB Compass hiển thị các tài liệu lưu vết lịch sử tự động hóa do vi phạm ngưỡng an toàn}
%     \label{fig:mongo-threshold}
% \end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{figures/mongo-control.png}
%     \caption{Giao diện MongoDB Compass hiển thị các tài liệu lưu vết lịch sử tương tác điều khiển thiết bị thủ công và bằng giọng nói}
%     \label{fig:mongo-control}
% \end{figure}

\subsubsection{Trạng thái cài đặt}
Hoàn thành một phần (Đã hiện thực hóa toàn bộ các tiến trình xử lý chạy nền phục vụ việc tự động lưu vết dữ liệu cảm biến chuỗi thời gian và lịch sử tương ước vào MongoDB; các service API truy vấn phân trang, tính năng xuất báo cáo và giao diện trực quan hóa dữ liệu lịch sử đang nằm trong lộ trình phát triển).

\subsubsection{Hạn chế và hướng phát triển}
Hạn chế lớn nhất của Module 4 nằm ở việc hệ thống giao diện tương tác và các API truy vấn lịch sử tương ứng công khai cho người dùng cuối vẫn còn bỏ ngỏ, khiến toàn bộ kho dữ liệu tài liệu NoSQL phong phú trong MongoDB bị cô lập. Để hiện thực hóa chức năng này, giải pháp triển khai ở tầng Backend là xây dựng các API định tuyến GET chuyên dụng, tích hợp các phương thức truy vấn nâng cao của Mongoose như \texttt{.skip()} và \texttt{.limit()} để thực hiện phân trang dữ liệu (\texttt{Pagination Request}), kết hợp bộ lọc \texttt{\$gte} và \texttt{\$lte} để lọc dữ liệu theo khoảng thời gian (\texttt{Time-range Filtering}) nhằm tối ưu hóa tốc độ phản hồi của cơ sở dữ liệu. Ở tầng Frontend, một trang chức năng độc lập mang tên Lịch sử hoạt động (\texttt{/history}) sẽ được xây dựng, tích hợp thư viện đồ họa \texttt{Recharts} để gọi các API phân trang nói trên, từ đó kéo dữ liệu chuỗi thời gian và kết xuất trực quan thành các dạng biểu đồ đường (Line Chart) biểu diễn xu hướng biến thiên môi trường.
...
\subsection{Yolo:Bit}
  \subsubsection{Giai đoạn khởi tạo (Initialization)}
\begin{itemize}
...
\subsubsection{Luồng thu thập dữ liệu và Giao tiếp Serial (Main Loop)}
...
\subsubsection{Luồng xử lý sự kiện (Event Manager)}
...
\subsubsection{Luồng đồng bộ trạng thái định kỳ (Timer Event)}
\begin{itemize}
